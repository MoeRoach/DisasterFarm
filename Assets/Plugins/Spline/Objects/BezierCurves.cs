...
namespace Pixelplacement
{
    public static class BezierCurves
    {
        ...
        public static Vector3 GetPoint (Vector3 startPosition, Vector3 endPosition, Vector3 startTangent, Vector3 endTangent, float percentage, bool evenDistribution, int distributionSteps)
        {
            if (evenDistribution)
            {
                var maxPoint = distributionSteps + 1;
                var arcLengths = new float[maxPoint];
                ...
                var targetLength = percentage * arcLengths[distributionSteps];

                //search:
                var low = 0;
                var high = distributionSteps;
                var index = 0;
                while (low < high)
                {
