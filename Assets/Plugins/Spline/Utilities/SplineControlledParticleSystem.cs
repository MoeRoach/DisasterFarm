...
public class SplineControlledParticleSystem : MonoBehaviour
{
    ...
    void LateUpdate ()
    {
        ...
        var aliveParticlesCount = _particleSystem.GetParticles (_particles);

        for (var i = 0; i < aliveParticlesCount; i++)
        {
            //get calculation pieces:
            var seedMax = Mathf.Pow(10, _particles[i].randomSeed.ToString().Length);
            var seedAsPercent = _particles[i].randomSeed / seedMax;
            var travelPercentage = 1 - (_particles[i].remainingLifetime / _particles[i].startLifetime);
            ...
            var offshootDirection = Quaternion.AngleAxis(1080 * seedAsPercent, -_spline.GetDirection(travelPercentage, false)) * _spline.Up(travelPercentage);
            var previousOffshootDirection = Quaternion.AngleAxis(1080 * seedAsPercent, -_spline.GetDirection(travelPercentage - _previousDiff, false)) * _spline.Up(travelPercentage - _previousDiff, false);
            ...
            var offset = Mathf.Lerp(startRadius, endRadius, travelPercentage);
            var previousOffset = Mathf.Lerp(startRadius, endRadius, travelPercentage - _previousDiff);
